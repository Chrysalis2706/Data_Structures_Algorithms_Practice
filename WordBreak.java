...
//                    last condition
    ...
    public static boolean wordBreak(String key){
        ...
        for (int i=1; i<=key.length(); i++){ //started from one bcz in substring method next bound is not included
            if (search(key.substring(0, i)) /*partion 1*/ && wordBreak(key.substring(i)) /*partion 2*/){
                return true;
            }
        }
        return false;
